    ...
    public void play() {
        pool.execute(() -> {
            ...
            while(offset < decodedSrc.length) {
                offset += refLine.write(decodedSrc, offset, offset + PLAY_CACHE_LENGTH > decodedSrc.length?decodedSrc.length - offset:PLAY_CACHE_LENGTH);
            }
            refLine.drain();
            refLine.stop();
            refLine.close();
